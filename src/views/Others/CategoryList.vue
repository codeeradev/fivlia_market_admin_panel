...
<template>
  <AdminLayout>
    ...
    <div
      v-if="alertMsg"
      class="mb-4 px-4 py-3 rounded text-sm font-medium"
      :class="
        alertType === 'success'
    ...
    >
      ...
      <div class="flex justify-between items-center mb-6">
        ...
        <div class="flex gap-2">
          ...
          <button
            v-if="viewMode === 'subcategories'"
            @click="openCategoryModal('sub', null, parentCategory?._id)"
      ...
      <div class="overflow-x-auto">
        <table class="min-w-full text-left border border-gray-200 dark:border-gray-700 rounded-lg">
          ...
          <tbody v-if="viewMode === 'categories'">
            ...
            >
              ...
              <td class="px-4 py-2 text-center space-x-2">
                <button
                  @click="openCategoryModal('main', cat)"
          ...
          <tbody v-else>
            <tr
              v-for="sub in parentCategory?.subcat ?? []"
              :key="sub._id"
              class="border-t hover:bg-gray-50 dark:hover:bg-gray-800"
            >
              <td class="px-4 py-2">
              ...
              <td class="px-4 py-2 text-center">
                <button
                  @click="toggleStatus(parentCategory!._id!, sub._id)"
                  class="px-2 py-1 text-xs rounded text-white"
                  :class="sub.status ? 'bg-green-500' : 'bg-red-500'"
                >
              ...
              <td class="px-4 py-2 text-center space-x-2">
                ...
                <button
                  @click="deleteEntity(parentCategory!._id!, sub._id)"
                  class="px-2 py-1 bg-red-500 hover:bg-red-600 text-white text-xs rounded"
                >
                  Delete
                </button>
            ...
            <tr v-if="(parentCategory?.subcat ?? []).length === 0">
              <td colspan="5" class="text-center py-6 text-gray-500">No subcategories yet.</td>
            </tr>
          </tbody>
        </table>
      </div>
      ...
      <div
        v-if="viewMode === 'categories' && categories.length === 0"
    ...
    <Modal v-if="showCategoryModal" @close="closeCategoryModal">
      ...
      <div class="space-y-4">
        ...
        <!-- CATEGORY TYPE -->
        <div v-if="categoryType === 'main'" class="flex items-center gap-6 mt-2">
          <span class="text-sm font-medium">Category Type:</span>

          <label class="flex items-center gap-1 text-sm">
            <input type="radio" value="products" v-model="form.type" />
            Products
          </label>

          <label class="flex items-center gap-1 text-sm">
            <input type="radio" value="service" v-model="form.type" />
            Service
          </label>

          <label class="flex items-center gap-1 text-sm">
            <input type="radio" value="living" v-model="form.type" />
            Living
          </label>
        </div>
        ...
        <div>
          <label class="block mb-1 text-sm font-medium"> Attributes (comma separated) </label>
          <input
      ...
      <template #footer>
        <div class="flex justify-end gap-2">
          <button @click="closeCategoryModal" class="px-4 py-2 border rounded">Cancel</button>
          <button
            @click="saveCategory"
            :disabled="loading"
            class="px-4 py-2 rounded text-white flex items-center gap-2 justify-center"
...
const form = reactive({
  ...
  status: true,
  attribute: [] as string[],
  type: 'products' as 'products' | 'service',
})
...
const fetchCategories = async () => {
  try {
    ...
    if (viewMode.value === 'subcategories' && parentCategory.value) {
      const updatedParent = categories.value.find((c) => c._id === parentCategory.value?._id)
...
const openCategoryModal = (
  ...
  if (cat) {
    ...
    form.type = (cat as any).type || 'products'
  ...
  if (type === 'sub') {
    selectedParentId.value = parentId || parentCategory.value?._id || ''
  } else {
    selectedParentId.value = ''
  }
  if (cat) {
    cityList.value = JSON.parse(JSON.stringify((cat as any).city ?? []))
  } else {
    cityList.value = []
  }
}
  ...
  try {
    ...
    if (categoryType.value === 'main') {
      ...
      if (!selectedParentId.value) {
        alertMsg.value = 'Please select parent category'
        alertType.value = 'error'
        loading.value = false
        return
      }
    ...
    alertMsg.value = 'Saved successfully ✅'
