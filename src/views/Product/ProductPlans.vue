  ...
  <AdminLayout>
      ...
      <div v-else class="overflow-x-auto">
        <BaseTable>
          <template #head>
            ...
            <th class="px-4 py-2">Duration (Months)</th>
    ...
    <BaseModal v-if="showModal" @close="closeModal">
      ...
      <div class="grid grid-cols-1 gap-4">
        ...
        <div>
          <label class="font-medium text-sm mb-1 block">Duration (Months)</label>
          <input
...
const openEditModal = (plan) => {
  ...
  modalError.value = "";
  form.value = {
    duration: Number.isFinite(Number(plan?.duration)) ? Number(plan.duration) : "",
...
const savePlan = async () => {
  ...
  const parsedDuration = Number(form.value.duration);
  if (!Number.isFinite(parsedDuration) || parsedDuration <= 0) {
    modalError.value = "Valid duration is required";
    return;
  }
